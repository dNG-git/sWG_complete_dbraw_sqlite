...
----------------------------------------------------------------------------
$Id: swg_dbraw_sqlite.php,v 1.1 2008/12/25 18:27:18 s4u Exp $
#echo(sWGdbrawSqliteVersion)#
sWG/#echo(__FILEPATH__)#
----------------------------------------------------------------------------
NOTE_END //n*/
/**
* We need a unified interface to be used with SQL-compatible databases. This
* one is designed for SQLite.
*
...
if (defined ("CLASS_direct_dbraw_sqlite")) { $g_continue_check = false; }
...
*/
	/*#ifndef(PHP4) */public /* #*/function connect ()
	{
		...
		elseif ((function_exists ("sqlite_popen"))&&(strlen ($direct_settings['db_dbfile']))&&(strlen ($direct_settings['db_dbfilemode'])))
		{
			if ($direct_settings['db_peristent']) { $this->resource = @sqlite_popen ($direct_settings['db_dbfile'],(intval ($direct_settings['db_dbfilemode'],8)),$f_error); }
...
*/
	/*#ifndef(PHP4) */public /* #*/function disconnect ()
	{
		...
		if (is_resource ($this->resource))
		{
			...
			else { $f_return = sqlite_close ($this->resource); }
		}
...
*/
	/*#ifndef(PHP4) */public /* #*/function query_exec ($f_answer,$f_query)
	{
		...
		if (is_resource ($this->resource))
		{
			$f_result_pointer = sqlite_query ($f_query,$this->resource);

			if ($f_answer == "ar") { $f_return = sqlite_changes ($this->resource); }
			...
			elseif ($f_answer == "ma")
			{
				...
				while ($f_row_array = sqlite_fetch_array ($f_result_pointer,SQLITE_ASSOC))
				{
			...
			elseif ($f_answer == "ms")
			{
				...
				while ($f_row_array = sqlite_fetch_array ($f_result_pointer,SQLITE_NUM))
				{
					if (!empty ($f_row_array)) { $f_return[] = implode ("\n",$f_row_array); }
				}
			}
			elseif ($f_answer == "nr") { $f_return = sqlite_num_rows ($f_result_pointer); }
			elseif ($f_answer == "sa")
			{
				$f_return = array ();
				$f_row_array = sqlite_fetch_array ($f_result_pointer,SQLITE_ASSOC);
			...
			elseif ($f_answer == "ss")
			{
				$f_row_array = sqlite_fetch_array ($f_result_pointer,SQLITE_NUM);
